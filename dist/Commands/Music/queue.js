...
exports.default = {
    ...
    async execute(interaction) {
        ...
        const queues = player.queue.db.data.queue?.[interaction.guild.id]; // @ts-ignore
        ...
        playlist_stringMap.push(
            `재생중 :: [${nowMusic.title}](${nowMusic.url}) (${(0, getMiniteSecond_1.default)(
                nowMusic.duration
            )})`
        );
        playlist_stringMap.push(
            `현재 구간 : [${lapse_minutes}:${lapse_seconds}](${nowMusic.url}&t=${resultSecond}s)`
        );
        if (queues) {
            playlist_stringMap.push(' ');
            for (let i = 0; i < (queues.length > 10 ? 10 : queues.length); i++) {
                playlist_stringMap.push(
                    `${i + 1}. [${queues[i].title}](${queues[i].url}) (${(0,
                    getMiniteSecond_1.default)(queues[i].duration)})`
                );
            }
        }
