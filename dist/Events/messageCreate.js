        ...
        if (process.env.MUSIC_CHANNEL_ID.split(' ').includes(message.channel.id)) {
            if (!message.author.bot) {
                ...
                if (search.loadType == 'playlist') {
                    ...
                    for (
                        let i = 0;
                        i < (search.tracks.length > 10 ? 10 : search.tracks.length);
                        i++
                    ) {
                        selectmenu.addOptions(
                            new discord_js_1.StringSelectMenuOptionBuilder({
                                value: `${i}`,
                                label: `${i + 1}. ${search.tracks[i].title.slice(0, 80)} (${(0,
                                getMiniteSecond_1.default)(search.tracks[i].duration)})`,
                            })
                        );
                    }
                            ...
                            if (player.playing) {
                                message.channel
                                    .send({
                                        embeds: [],
                                        components: [],
                                        content: `**${search.tracks[selectMusicIndex].title} (${(0,
                                        getMiniteSecond_1.default)(
                                            search.tracks[selectMusicIndex].duration
                                        )}) 을/를 추가합니다**`,
                                    })
                                    .then((f) => {
                                        // setTimeout(() => {
            ...
            if (message.author.bot) {
                setTimeout(() => {
                    ...
                    if (message.id == dj_message.get(message.guild.id)) return;
